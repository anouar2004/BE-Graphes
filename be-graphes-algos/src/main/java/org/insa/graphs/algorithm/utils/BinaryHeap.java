...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    public void remove(E x) throws ElementNotFoundException {
        int index = -1;
        // on regarde si on trouve l'élément à supprimer
    ...
    // 2. Si l'élément n'existe pas, on lève une exception
    if (index == -1) {
        throw new ElementNotFoundException(x);
    }

    // 3. Récupérer le dernier élément et réduire la taille
    E lastItem = this.array.get(this.currentSize - 1);
    this.currentSize--;

    // Cas particulier : si on a supprimé le dernier élément lui-même, rien à faire
    if (index == this.currentSize) {
        return;
    }

    // 4. Placer le dernier élément à la place de celui supprimé
    this.arraySet(index, lastItem);

    // 5. Restaurer l'ordre du tas
    // On essaie d'abord de le faire monter
    if (index > 0 && this.array.get(index).compareTo(this.array.get(indexParent(index))) < 0) {
        this.percolateUp(index);
    } 
    // Sinon, on essaie de le faire descendre
    else {
